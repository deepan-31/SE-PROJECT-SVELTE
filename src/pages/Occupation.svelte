<script>
  ...
  onMount(async () => {
    ...
    const reservationsResponse = await fetch(
      "http://localhost:4000/users/reservations"
    );
    const timetablesResponse = await fetch(
      "http://localhost:4000/users/timetables"
    );
...
<main class="container">
  <button
    ...
    on:click={generateOccupancyChart}>Generate Occupancy Chart</button
  >
          ...
          <tbody>
            {#each timetables as timetable}
              <tr>
                ...
                <td
                  class:occupied={getStatus(
                    timetable.room_id,
                    timetable.day,
                    timetable.slot
                  ) === "Occupied"}
                  class:available={getStatus(
                    timetable.room_id,
                    timetable.day,
                    timetable.slot
                  ) === "Available"}
